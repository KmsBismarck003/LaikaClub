...
# ==========================================
# PLAN DE INVIERNO - MYSQL DISASTER RECOVERY
# ==========================================
...
def main():
    ...
    # Si no existe en la ruta del env, intentar buscarlo en PATH de Windows o rutas comunes de XAMPP/WAMP
    if not os.path.exists(mysql_exe):
        ...
        for path in common_paths:
            if os.path.exists(path) or path in ["mysql.exe", "mysql"]:
                mysql_exe = path
                found = True
                break
        if not found:
    ...
    # Si no hay respaldos y existe el script inicial, podemos usar el sql inicial como fallback
    setup_sql = 'laika_club_database.sql'
    if not sql_files and os.path.exists(setup_sql):
        sql_files.append(setup_sql)

    if not sql_files:
        print("[⚠️] ADVERTENCIA: No se encontraron archivos .sql de respaldo ni base de datos inicial.")
    ...
    if action == '--check':
        ...
        mysqladmin_exe = env.get('MYSQLADMIN_EXE_PATH', r'C:\xampp\mysql\bin\mysqladmin.exe')
        if not os.path.exists(mysqladmin_exe):
            mysqladmin_exe = mysql_exe.replace("mysql.exe", "mysqladmin.exe").replace("mysql", "mysqladmin")
            
        create_db_cmd = f'"{mysqladmin_exe}" -h {db_host} -u {db_user} '
        if db_pass:
            create_db_cmd += f'-p{db_pass} '
        create_db_cmd += f'create {db_name}'
        
        # Ejecutar silenciosamente
        subprocess.run(create_db_cmd, shell=True, capture_output=True)

        # Comando de restauración
        cmd = f'"{mysql_exe}" -h {db_host} -u {db_user}'
        if db_pass:
            cmd += f' -p{db_pass}'
