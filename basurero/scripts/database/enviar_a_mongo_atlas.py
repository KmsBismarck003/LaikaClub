...
def main():
    ...
    # ===== RESUMEN FINAL =====
    print_sep("ESTADO DEL ECOSISTEMA NOSQL (10 COLECCIONES)")
    critical_list = [
    ...
    colecciones_actuales = mongo_db.list_collection_names()
    for col_name in critical_list:
        status = "[ OK ]" if col_name in colecciones_actuales else "[MISSING]"
        count = mongo_db[col_name].count_documents({}) if col_name in colecciones_actuales else 0
        print(f"  {status} {col_name:25s}: {count} documentos")

    print("\n[LISTO] Sync completado. Ecosistema NoSQL verificado en Atlas!")
    conn.close()
    client.close()
