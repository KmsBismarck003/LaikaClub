...
- Ver todas las transacciones (boletos + merchandising)
...
### 🎯 Radar de Operaciones (`/admin/radar`) ⭐ NUEVO
Monitor en tiempo real del mapa de asientos del venue:
- Visualización del plano del recinto con estado de cada asiento
- Colores por estado: **libre** (verde) / **ocupado** (rojo) / **reservado** (amarillo) / **bloqueado** (gris)
- Estadísticas de ocupación por zona en el panel lateral
- Útil para coordinación logística y control de aforo durante el evento
- Actualización automática sin necesidad de recargar la página

---

### 🛍️ Aprobación de Merchandising (`/admin/merchandise-approval`) ⭐ NUEVO
Flujo de aprobación para los productos creados por los gestores:
- Ver todos los productos en estado **"Pendiente"** de los gestores
- **Aprobar** → El producto se publica en la sección de merch del evento
- **Rechazar** → Se notifica al gestor con comentario opcional
- Garantiza control de calidad y consistencia del catálogo

---

### 🏪 Gestión Global de Merch (`/admin/merch`) ⭐ NUEVO
Catálogo global de merchandising administrado directamente por el admin:
- Crear productos disponibles para cualquier evento
- Editar precios, imágenes, tallas y descripciones
- Activar o desactivar productos del catálogo
...
| **gestor**   | Eventos, ventas, analytics, asistentes, merch propia (sin acceso a sistema)|
