...
def train_ridge_regression(train_df, test_df, evaluator, mongo_db=None):
    """
    Entrena un modelo de Regresión Ridge (L2 Regularization) sobre PySpark.
    ...
    # regParam = 0.5, elasticNetParam = 0.0 define la penalización L2 (Ridge)
    ridge = LinearRegression(featuresCol="features", labelCol="ingreso", regParam=0.5, elasticNetParam=0.0)
    model = ridge.fit(train_vector)
    ...
    if mongo_db is not None:
        try:
            ...
            print("[MONGO SAVE] Ridge Regression metrics stored in MongoDB.")
