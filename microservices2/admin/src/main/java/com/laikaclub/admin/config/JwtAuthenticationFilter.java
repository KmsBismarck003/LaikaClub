...
import com.laikaclub.admin.service.TokenService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@org.springframework.lang.NonNull HttpServletRequest request,
        ...
        // First check header overrides for local development/testing
        ...
        if (xUserId != null && xUserRole != null) {
            ...
            // Otherwise parse JWT standard token
            String authHeader = request.getHeader("Authorization");
            System.out.println("[ADMIN SECURITY] Received Authorization header: " + authHeader);
            if (authHeader != null && authHeader.startsWith("Bearer ")) {
                String token = authHeader.substring(7).trim();
                boolean isValid = tokenService.validateToken(token);
                System.out.println("[ADMIN SECURITY] Token is valid: " + isValid);
                if (isValid) {
