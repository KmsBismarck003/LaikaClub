...
def log(msg):
    with open("import_db_v3.log", "a", encoding="utf-8") as f:
...
if os.path.exists("import_db_v3.log"):
    os.remove("import_db_v3.log")

log(f"Starting database import process for {db_name} (CLEAN START)...")

# Drop database
log(f"Dropping database {db_name} if it exists...")
subprocess.run([mysqladmin_exe, "-u", "root", "drop", db_name, "-f"], capture_output=True)
